...
UAT-CHECK( )
- **Ghost**: When cursor is on a line with markdown syntax but NOT inside the construct - markers show at 50% opacity 
UAT-CHECK( )
...
UAT-CHECK( )
- [ ] **Ghost state**: When cursor is on the same line but outside the construct, markers show faintly (50% opacity) 
UAT-CHECK( )
